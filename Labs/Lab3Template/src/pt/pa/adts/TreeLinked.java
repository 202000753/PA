...
public class TreeLinked<E> implements Tree<E> {
    ...
    @Override
    public boolean isAncestor(Position posDesc, Position posAsc) {
        TreeNode desc = checkPosition(posDesc);
        TreeNode asc = checkPosition(posAsc);

        for (TreeNode node:asc.children) {
            if(node == desc)
                return true;
            else {
                if(isAncestor(desc, node))
                    System.out.println("Is Acenstor??");
            }
        }
    ...
    @Override
    public void move(Position existingPosition, Position newParent) {
        TreeNode existing = checkPosition(existingPosition);
        TreeNode parent = checkPosition(newParent);

        E element = this.remove(existing);

        this.insert(parent, element);
    }
