...
public class IntegerBST implements IntegerBinarySearchTree {
    ...
    public int sum() throws EmptyContainerException {
        ...
        this.inOrder(this.root, elements);
    ...
    /*public boolean isInternal(int element) {
        TreeNode node = getNode(element, this.root);
    ...
    private TreeNode getNode(int element, TreeNode node) {
        if (node == null) {
            return null;
        }
        ...
        if (element < node.element) {
            System.out.println("AA "+element+" "+node.element);
            return getNode(element, root.left);
        } else {
            return getNode(element, root.right);
        }
    }*/
    ...
    public int sumInternals() throws EmptyContainerException {
        ...
        return sumInternals(root.left) + sumInternals(root.right);
    }

    private int sumInternals(TreeNode root) {
        int sum = 0;

        if(this.isInternal(root))
        {
            sum+=root.element + sumInternals(root.left) + sumInternals(root.right);
        }
